...
CREATE TABLE `seasons` (
  ...
  `drivers_championship_leader_id` INT DEFAULT null,
  `constructors_championship_leader_id` INT DEFAULT null,
  `season_start_date` DATE,
  `season_end_date` DATE,
  `total_points_awarded` DECIMAL(10,1) DEFAULT 0,
  `created_at` TIMESTAMP DEFAULT (CURRENT_TIMESTAMP)
);
...
CREATE TABLE `races` (
  ...
  `weather_conditions` ENUM ('Clear', 'Partly Cloudy', 'Overcast', 'Sunny', 'Rain', 'Hot', 'Variable', 'Hot & Humid', 'Rain', 'Cool') DEFAULT 'Clear',
...
CREATE TABLE `disqualifications` (
  `dsq_id` INT PRIMARY KEY AUTO_INCREMENT,
  `driver_id` INT NOT NULL,
  `race_id` INT NOT NULL,
  `disqualification_reason` VARCHAR(200)
);

CREATE TABLE `did_not_finishes` (
  `dnf_id` INT PRIMARY KEY AUTO_INCREMENT,
  `driver_id` INT NOT NULL,
  `race_id` INT NOT NULL,
...
ALTER TABLE `tracks` ADD FOREIGN KEY (`lap_record_holder_id`) REFERENCES `drivers` (`driver_id`);

ALTER TABLE `tracks` ADD FOREIGN KEY (`country_id`) REFERENCES `countries` (`country_id`);

ALTER TABLE `seasons` ADD FOREIGN KEY (`drivers_championship_leader_id`) REFERENCES `drivers` (`driver_id`);

ALTER TABLE `seasons` ADD FOREIGN KEY (`constructors_championship_leader_id`) REFERENCES `teams` (`team_id`);
...
ALTER TABLE `races` ADD FOREIGN KEY (`fastest_lap_driver_id`) REFERENCES `drivers` (`driver_id`);

ALTER TABLE `races` ADD FOREIGN KEY (`pole_position_driver_id`) REFERENCES `drivers` (`driver_id`);

ALTER TABLE `races` ADD FOREIGN KEY (`winner_driver_id`) REFERENCES `drivers` (`driver_id`);
...
ALTER TABLE `race_results` ADD FOREIGN KEY (`team_id`) REFERENCES `teams` (`team_id`);
...
ALTER TABLE `disqualifications` ADD FOREIGN KEY (`driver_id`) REFERENCES `drivers` (`driver_id`);

ALTER TABLE `disqualifications` ADD FOREIGN KEY (`race_id`) REFERENCES `races` (`race_id`);

ALTER TABLE `did_not_finishes` ADD FOREIGN KEY (`race_id`) REFERENCES `races` (`race_id`);

ALTER TABLE `did_not_finishes` ADD FOREIGN KEY (`driver_id`) REFERENCES `drivers` (`driver_id`);
