...
(fit1 <- Arima(bluebird[,2], order = c(0,0,1)))
  # vyzkousejte ruzne modely, toto je MA(1)
AIC(fit1)
  # Akaikeho informacni kritarium
BIC(fit1)
  # Bayesovske informacni kriterium

(fit2 <- Arima(bluebird[,2], order = c(1,0,0)))
(fit3 <- Arima(bluebird[,2], order = c(1,0,1)))
(fit4 <- Arima(bluebird[,2], order = c(2,0,0)))
(fit5 <- Arima(bluebird[,2], order = c(0,0,2)))
(fit6 <- Arima(bluebird[,2], order = c(2,0,2)))

# Ktery model je nejlepsi?

Box.test(residuals(fit1), lag=5, type = "Ljung-Box")
  # test o nulove autokorelaci
Box.test(residuals(fit2), lag=5, type = "Ljung-Box")

(fit.a <- auto.arima(bluebird[,2]))
  # automaticky hledani modelu
coeftest(fit.a)
  # test o nulovosti koeficientu modelu
  # model ma nevyznamne cleny, je tedy prilis komplikovany
###########################
...
par(mfrow = c(1,1))
acf(BJsales, na.action = na.pass, type = "covariance", plot = F)
...
(fit.a.d1 <- auto.arima(BJsales, seasonal = T, ic = "bic", d = 1))
(fit.a.d2 <- auto.arima(BJsales, seasonal = T, ic = "bic", d = 2))
(fit.a <- auto.arima(BJsales, seasonal = T, ic = "bic"))

(fit1 <- Arima(d2, order = c(1,0,0), seasonal = list(order = c(0,1,0))))
(fit2 <- Arima(d2, order = c(0,0,1), seasonal = list(order = c(0,1,0))))
(fit3 <- Arima(d2, order = c(1,0,1), seasonal = list(order = c(0,1,0))))
(fit4 <- Arima(d2, order = c(0,0,0), seasonal = list(order = c(1,1,0))))
(fit5 <- Arima(d2, order = c(0,0,0), seasonal = list(order = c(0,1,1))))
(fit6 <- Arima(d2, order = c(0,0,0), seasonal = list(order = c(1,1,1))))
(fit7 <- Arima(d2, order = c(1,0,0), seasonal = list(order = c(1,1,1))))
BIC(fit1)
BIC(fit2)
BIC(fit3)
BIC(fit4)
