...
(fit5 <- arima(nhtemp, order = c(0,0,2)))
  # rada MA(2)
(fit6 <- arima(nhtemp, order = c(1,0,1)))
  # rada ARMA(1, 1)
(fit7 <- arima(nhtemp, order = c(2,0,1)))
  # rada ARMA(2, 1)
# podle AIC kriteria je nejvhodnejsi rada ARMA(1, 1)
  # z vystupu dostavame odhad Y(t) = 51.17 + 0.92Y(t-1) - 0.71e(t-1) + e(t)
plot(nhtemp)
...
# rada MA(2)
(fit6 <- arima(LakeHuron, order = c(1,0,1)))
# rada ARMA(1, 1)
(fit7 <- arima(LakeHuron, order = c(2,0,1)))
# rada ARMA(2, 1)
(fit7 <- arima(LakeHuron, order = c(1,0,2)))
# rada ARMA(1, 2)
