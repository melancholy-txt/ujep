...
t.test(cislo ~ kategorie, var.eq = T)
  ...
  #ttest s onesided alternativou
  t.test(cislo ~ kategorie, mu=0, alternative="greater", var.eq = T)
  # p-hodnota = 0.002756 < alfa (0.05) -> zamitame H0, plati H1
  # porodni hmotnost hochu je v prumeru vetsi nez porodni hmotnost divek
...
t.test(cislo ~ kategorie, var.eq = T, alternative = "less")
  # p-hodnota = 0.0048 < alfa (0.05) -> zamitame H0, plati H1
  # pulrocni kluci jsou v prumeru tezsi nez pulrocni divky
...
t.test(cislo ~ kategorie, var.eq = T, alternative = "two.sided")
## Je rozdil ve veku matek, ktere jeste v pul roce kojily a tech co nekojily? (promenne vekM, Koj24)

cislo <- Kojeni$hmotnost
kategorie <- Kojeni$Vzdelani
boxplot(cislo ~ kategorie)

PlotQQ(residuals(lm(cislo ~ kategorie)))
